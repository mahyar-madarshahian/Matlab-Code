...
n = length(mashhad_graph);
k = 1; % shomarandeye x(k)
z = 1; % shomarandeye a(z)
for i = 1:n
    for j = 1:n
        if mashhad_graph(i,j) == 0
            mashhad_graph(i,j) = NaN;
        elseif mashhad_graph(i,j) == Inf
           mashhad_graph(i,j) = NaN; 
        end
    end
end
...
for i = 1:n % shomare sotoon
    for j = 1:n % shomare satr
        if strcmp(num2str(mashhad_graph(j,i)),'NaN') ~= 1
           x(k) = j;
           a(z) = mashhad_graph(j,i); % addade deraye motanazer ba find
           k = k+1;
           z = z+1;
        end
    end
    ...
    for H = 1:n % shomarandeye satr baraye sotoone i , baraye peyda kardane tedad derayehayi ke min hastan
        if mashhad_graph(H,i) == min(a)
           flag = [flag,H]; % derayehaye motanazer ba min(a) dar mashhad_graph
        end
    end
        ...
        if   isempty(path) == 0
             ...
             for W = 1:length(bru_masir{G})-1
                 for R = W+1:length(bru_masir{G})
                     mashhad_graph(bru_masir{G}(1,W),bru_masir{G}(1,R)) = NaN;
                     mashhad_graph(bru_masir{G}(1,R),bru_masir{G}(1,W)) = NaN;
                 end
             end
             k = k+1;
             G = G+1;
...
for i = 1:length(barg_tree)
    for j = 1:length(barg_tree(i,:))
        ...
        for k = 1:length(barg_search)
            for z = 1:length(mashhad_graph)
                if strcmp(num2str(barg_search(k)),'NaN') ~= 1
                   if strcmp(num2str(mashhad_graph(z,barg_search(k))),'NaN') ~= 1
                       if isempty(barg_search1) == 0 ;
                           ...
                           for R = 1:length(barg_masir)
                               for RR = 1:length(bru_masir{barg_masir(R)})
                           ...
                           if isempty(path1) == 1  
                                 if strcmp(num2str(pa),'NaN') ~= 1
                                                   if mashhad_graph(z,barg_search(k))< barg_search1(1,:)
                                                      clear barg_new
                                                      barg_search1 = [mashhad_graph(z,barg_search(k))];% inja az halate tasadofi dar miad, khodam dar ayande behboodesh midam
                            ...
                            barg_search1 = [barg_search1,mashhad_graph(z,barg_search(k))];
        ...
        [skims, back_nodes] = generate_skims(brouvka); %% chek shavad ke masir doros ast??
        mashhad_graph(barg_new(1),barg_new(2)) = NaN;
        mashhad_graph(barg_new(2),barg_new(1)) = NaN;
        clear barg_search
        barg_search = [];
        clear barg_search1
        barg_search1 = [];
...
clear bru_masir barg_tree barg barg_masir barg_new barg_search barg_search1 terminals
terminals = [];
k = 1;
i = 1;
n = length(mashhad_graph);
...
for E = 1:n-1
    for A = E+1:n
        pa = path_search(terminals(E), terminals(A),back_nodes);
        path = es_path(pa,terminals(E));
        if isempty(path) == 0
            if path(end) ~= terminals(A)
               clear path
