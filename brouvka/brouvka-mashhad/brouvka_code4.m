                       ...
                       if isempty(barg_search1) == 0 ;
                           ...
                           for R = 1:length(barg_masir)
                               for RR = 1:length(bru_masir{R})
                                   ...
                                   pa = path_search(barg_masir(R), bru_masir{barg_masir(R)}(1,RR), back_nodes);
                                   path = es_path(pa,z);
                                   
                                   %%% inja ye moshkeliiiiiiiii hast,
                                   %%% back_nodes shortest path misaze!!!!
                                   clear xxx
                                   xxx = [];
                                      for RRR = 1:length(barg_tree)
                                         if find(isempty((setdiff(path,bru_masir{RRR}(1,:)))) == 1)
                                         xxx = 1;
                                         break
                                         end
                                      end
