                   ...
                   if strcmp(num2str(kashan_graph(z,barg_search(k))),'NaN') ~= 1
                       if isempty(barg_search1) == 0 ;
                           ...
                           for L = 1:length(barg_tree)
                               if find(bru_masir{L}(1,:) == barg_search(k))
                                   barg_masir = [barg_masir,L];
                               end
                           end
                                   for R = 1:length(barg_tree)
                                       if find(bru_masir{R} == z)
                                          bargg_search = [bargg_search,bru_masir{R}(1,1)];
                                          bargg_search = [bargg_search,bru_masir{R}(1,end)];
                                          for p = 1:length(bargg_search)
                                              for M = 1:length(barg_masir)
                                                 if brouvka(barg_search(p),barg_masir(M)) == 1
                                                     break
                                                 else
